...
namespace Microsoft.Framework.PackageManager
{
    public class DnuCommandsTests
    {
        ...
        public void DnuCommands_Uninstall_RemovesUnusedPackageNotUsedByApp(string flavor, string os, string architecture)
        {
            ...
            {
                InstallFakeApp(testEnv.RootDir, "pack1", "0.0.0");
                Directory.CreateDirectory($"{testEnv.RootDir}/packages/pack2/0.0.0/");
                //InstallFakeApp(testEnv.RootDir, "pack2", "0.0.0");
                WriteLockFile($"{testEnv.RootDir}/packages/pack1/0.0.0/app", "pack1", "0.0.0");
                ...
                Assert.False(Directory.Exists($"{testEnv.RootDir}/packages/pack2"));
            }
        }
        ...
        public void DnuCommands_Uninstall_NoPurge_DoesNotRemoveUnusedPackageNotUsedByApp(string flavor, string os, string architecture)
        {
            ...
            using (var testEnv = new DnuTestEnvironment(runtimeHomeDir))
            {
                ...
                var command = new UninstallCommand(repo, reports);
                command.NoPurge = true;

                command.Execute("pack1");

                //Pack1 only used by pack1 app but --no-purge should not remove it
                Assert.True(Directory.Exists($"{testEnv.RootDir}/packages/pack1"));
