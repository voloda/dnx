...
namespace Microsoft.Framework.PackageManager
{
  ...
  Using Project dependency {0} 1.0.0
    Source: {1}{2}project.json";
            string expectedDNXCore =
            @"Building {0} for DNXCore,Version=v5.0
  Using Project dependency {0} 1.0.0
    Source: {1}{2}project.json";
            string expectedNupkg =
            @"{0} -> {1}{2}bin{2}Debug{2}{0}.1.0.0.nupkg
{0} -> {1}{2}bin{2}Debug{2}{0}.1.0.0.symbols.nupkg
...
";
            ...
            using (var testEnv = new DnuTestEnvironment(runtimeHomeDir))
            {
                ...
                Assert.Contains(string.Format(expectedDNX, Path.GetFileName(testEnv.RootDir), testEnv.RootDir, Path.DirectorySeparatorChar), stdOut);
                Assert.Contains(string.Format(expectedDNXCore, Path.GetFileName(testEnv.RootDir), testEnv.RootDir, Path.DirectorySeparatorChar), stdOut);
                Assert.Contains(string.Format(expectedNupkg, Path.GetFileName(testEnv.RootDir), testEnv.RootDir, Path.DirectorySeparatorChar), stdOut);
...
";
        ...
        {
            string expectedNupkg =
            @"{0} -> {1}{2}CustomOutput{2}Debug{2}{0}.1.0.0.nupkg
{0} -> {1}{2}CustomOutput{2}Debug{2}{0}.1.0.0.symbols.nupkg
